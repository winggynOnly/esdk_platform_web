...
public class HTTPConduit extends AbstractConduit implements Configurable, Assertor, PropertyChangeListener
{
    ...
    */
    ...
    private void updateClientPolicy()
    {
        ...
        {
            clientSidePolicy = policyEngine.getClientEndpointPolicy(endpointInfo, this, new ClientPolicyCalculator());
        }
    }
    ...
    private static void configureConduitFromEndpointInfo(HTTPConduit conduit, EndpointInfo endpointInfo)
    {
        ...
        {
            conduit.setAuthorization(endpointInfo.getTraversedExtensor(new AuthorizationPolicy(),
                AuthorizationPolicy.class));
            
        }
        if (conduit.getProxyAuthorization() == null)
        {
            conduit.setProxyAuthorization(endpointInfo.getTraversedExtensor(new ProxyAuthorizationPolicy(),
    ...
    private void logConfig()
    {
        ...
        {
            LOG.log(Level.FINE, "Message Trust Decider of class '" + trustDecider.getClass().getName()
                + "' with logical name of '" + trustDecider.getLogicalName() + "' has been configured for Conduit '"
                + getConduitName() + "'");
        }
        ...
        {
            LOG.log(Level.FINE, "HttpAuthSupplier of class '" + authSupplier.getClass().getName()
                + "' has been configured for Conduit '" + getConduitName() + "'");
        }
        if (this.tlsClientParameters != null)
        {
            LOG.log(Level.FINE,
                "Conduit '" + getConduitName() + "' has been configured for TLS " + "keyManagers "
                    + Arrays.toString(tlsClientParameters.getKeyManagers()) + "trustManagers "
                    + Arrays.toString(tlsClientParameters.getTrustManagers()) + "secureRandom "
                    + tlsClientParameters.getSecureRandom() + "Disable Common Name (CN) Check: "
    ...
    protected void finalizeConfig()
    {
        ...
        {
            this.endpointInfo.setProperty("org.apache.cxf.ws.addressing.replyto", getClient().getDecoupledEndpoint());
        }
    ...
    private HttpURLConnection createConnection(Message message, URL url)
        throws IOException
    {
        ...
        throws IOException
    {
        ...
        //Added by zhili for disabling Cookie Management function
        ...
        if (this.authSupplier.requiresRequestCaching())
        {
            ...
            isChunking = false;
            LOG.log(Level.FINE, "Auth Supplier, but no Premeptive User Pass or Digest auth (nonce may be stale)"
        ...
        if (csPolicy.isAllowChunking() && isChunkingSupported(message, connection.getRequestMethod()))
        {
        ...
        message.setContent(OutputStream.class,
            createOutputStream(message, connection, needToCacheRequest, isChunking, chunkThreshold));
    ...
    protected boolean isChunkingSupported(Message message, String httpMethod)
    {
        ...
        if (HTTP_PUT_METHOD.equals(httpMethod))
        {
            ...
            if (objs != null && objs.size() > 0)
            {
                ...
                if (obj.getClass() != String.class || (obj.getClass() == String.class && ((String)obj).length() > 0))
                {
                    return true;
                }
            }
    ...
    protected OutputStream createOutputStream(Message message, HttpURLConnection connection,
        boolean needToCacheRequest, boolean isChunking, int chunkThreshold)
    {
        return new WrappedOutputStream(message, connection, needToCacheRequest, isChunking, chunkThreshold,
            getConduitName());
    }
        ...
        if (message.get(Message.CONNECTION_TIMEOUT) != null)
        {
            ...
            catch (NumberFormatException e)
            {
                LOG.log(Level.WARNING,
                    ...
                    new Object[] {Message.CONNECTION_TIMEOUT, obj.toString()});
            }
        }
        ...
        throws IOException
    {
        ...
        try
        {
            if (in != null)
            {
                ...
                while (in.read(buffer) != -1 && count < 25)
                {
     ...
     */
    private URL setupURL(Message message)
        throws MalformedURLException
    {
    ...
    private void setHeadersByAuthorizationPolicy(Message message, URL url)
    {
        ...
        String proxyAuthString = authSupplier.getAuthorization(proxyAuthorizationPolicy, url, message, null);
    ...
    public void setTlsClientParameters(TLSClientParameters params)
    {
        ...
        if (this.tlsClientParameters != null)
        {
            ...
            {
                LOG.log(Level.FINE,
                    "Conduit '" + getConduitName() + "' has been (re) configured for TLS " + "keyManagers "
                        + Arrays.toString(tlsClientParameters.getKeyManagers()) + "trustManagers "
                        + Arrays.toString(tlsClientParameters.getTrustManagers()) + "secureRandom "
     ...
     * @throws IOException
     */
    private HttpURLConnection processRetransmit(final HttpURLConnection origConnection, Message message,
     ...
     * @throws IOException
     */
    private HttpURLConnection redirectRetransmit(HttpURLConnection connection, Message message,
        CacheAndWriteOutputStream cachedStream)
        throws IOException
    {
     ...
     * @throws IOException
     */
    private HttpURLConnection authorizationRetransmit(HttpURLConnection connection, Message message,
        CacheAndWriteOutputStream cachedStream)
        throws IOException
    {
        ...
        String authorizationToken =
            authSupplier.getAuthorization(effectiveAthPolicy, currentURL, message, authHeader.getFullHeader());
     ...
     * @throws IOException
     */
    private HttpURLConnection retransmit(URL newURL, Message message, CacheAndWriteOutputStream stream)
        throws IOException
    {
        ...
        if (LOG.isLoggable(Level.FINE))
        {
            LOG.fine("Conduit \"" + getConduitName() + "\" Retransmit message to: " + connection.getURL() + ": "
                + new String(stream.getBytes(), "UTF-8"));
        }
        return connection;
    }
    
    private static void detectAuthorizationLoop(String conduitName, Message message, URL currentURL, String realm)
        throws IOException
    {
        ...
        {
            String logMessage =
                "Authorization loop detected on Conduit \"" + conduitName + "\" on URL \"" + currentURL
                    + "\" with realm \"" + realm + "\"";
     ...
     * @throws IOException
     */
    private static void detectRedirectLoop(String conduitName, URL lastURL, URL newURL, Message message)
        throws IOException
    {
        @SuppressWarnings("unchecked")
    ...
    protected class WrappedOutputStream extends AbstractThresholdOutputStream
    {
        ...
        protected WrappedOutputStream(Message outMessage, HttpURLConnection connection, boolean possibleRetransmit,
            boolean isChunking, int chunkThreshold, String conduitName)
        {
        ...
        public void thresholdReached()
        {
            if (chunking)
            {
                connection.setChunkedStreamingMode(HTTPConduit.this.getClient().getChunkLength());
            }
        }
            ...
            throws IOException
        {
            ...
            if (LOG.isLoggable(Level.FINE))
            {
                LOG.fine("Sending " + connection.getRequestMethod() + " Message with Headers to " + connection.getURL()
                    + " Conduit :" + conduitName + "\n");
            }
        }
            ...
            throws IOException
        {
            ...
            catch (IOException e)
            {
                ...
                throw mapException(e.getClass().getSimpleName() + " invoking " + connection.getURL() + ": "
                    + e.getMessage(),
                    e,
                    IOException.class);
            }
            ...
            throws IOException
        {
            ...
            if (cachedStream != null || ("GET".equals(connection.getRequestMethod()) && getClient().isAutoRedirect()))
            {
                ...
                {
                    LOG.fine("Conduit \"" + getConduitName() + "\" Transmit cached message to: " + connection.getURL()
                        + ": " + new String(cachedStream.getBytes(), "UTF-8"));
                }
                ...
                try
                {
                    ...
                    if (ex == null)
                    {
                        WorkQueueManager mgr =
                            outMessage.getExchange().get(Bus.class).getExtension(WorkQueueManager.class);
            ...
            throws IOException
        {
            ...
            boolean noExceptions =
                MessageUtils.isTrue(outMessage.getContextualProperty("org.apache.cxf.http.no_io_exceptions"));
        ...
        private void logResponseInfo(int responseCode)
        {
            if (LOG.isLoggable(Level.FINE))
            {
                ...
                if (null != headerFields)
                {
                    ...
                    for (Map.Entry<String, List<String>> item : headerFields.entrySet())
                    {
                        buf.append("    " + item.getKey() + ": " + item.getValue() + newLine);
                    }
                    
                    // replace cookie before log
    ...
    protected void closeInputStream(HttpURLConnection connection)
        throws IOException
    {
    ...
    {
        if (evt.getSource() == clientSidePolicy && "decoupledEndpoint".equals(evt.getPropertyName()))
        {
    ...
    {
        String msg =
            "HTTP response '" + e.responseCode() + ": " + connection.getResponseMessage() + "' invoking "
                + connection.getURL();
        switch (e.responseCode())
        {
            ...
            case HttpURLConnection.HTTP_PROXY_AUTH: // 407
                ...
                {
                    msg += " with proxy authorization username '" + proxyAuthorizationPolicy.getUserName() + "'";
                }
                if (clientSidePolicy == null || clientSidePolicy.getProxyServer() == null)
                {
                    ...
                    {
                        msg +=
                            " using a proxy even if NONE is configured in CXF conduit " + getConduitName()
                                + " (maybe one is configured by java.net.ProxySelector)";
                    }
                    else
                    {
                        msg +=
                            " but NO proxy was used by the connection (none configured in cxf "
                ...
                {
                    msg +=
                        " using " + clientSidePolicy.getProxyServerType() + " proxy "
                            + clientSidePolicy.getProxyServer() + ":" + clientSidePolicy.getProxyServerPort();
                }
                break;
            default:
