...
public class Headers
{
    ...
    public static final String PROTOCOL_HEADERS_CONTENT_TYPE = Message.CONTENT_TYPE.toLowerCase(Locale.US);
    ...
    void setFromClientPolicy(HTTPClientPolicy policy)
    {
        ...
        {
            headers.put("Cache-Control", createMutableList(policy.getCacheControl()));
        }
        if (policy.isSetHost())
        {
            headers.put("Host", createMutableList(policy.getHost()));
        }
        ...
        {
            headers.put("Accept", createMutableList(policy.getAccept()));
        }
        ...
        {
            headers.put("Accept-Encoding", createMutableList(policy.getAcceptEncoding()));
        }
        if (policy.isSetAcceptLanguage())
        {
            headers.put("Accept-Language", createMutableList(policy.getAcceptLanguage()));
        }
        ...
        {
            headers.put("Cookie", createMutableList(policy.getCookie()));
        }
        if (policy.isSetBrowserType())
        {
            headers.put("BrowserType", createMutableList(policy.getBrowserType()));
        }
        if (policy.isSetReferer())
        {
            headers.put("Referer", createMutableList(policy.getReferer()));
        }
    }
    ...
    {
        headers.put("Authorization", createMutableList(authorization));
    }
    
    public void setProxyAuthorization(String authorization)
    {
        headers.put("Proxy-Authorization", createMutableList(authorization));
    }
    ...
    public static Map<String, List<String>> getSetProtocolHeaders(final Message message)
    {
        ...
        {
            Map<String, List<String>> headers2 = new TreeMap<String, List<String>>(String.CASE_INSENSITIVE_ORDER);
    ...
    void logProtocolHeaders(Level level)
    {
        for (String header : headers.keySet())
        {
            ...
            for (String value : headerList)
            {
                ...
                LOG.log(level, header + ": " + logMessage);
            }
        }
    }
    ...
    private String determineContentType()
    {
        ...
        if (null != ct)
        {
            if (enc != null && ct.indexOf("charset=") == -1 && !ct.toLowerCase(Locale.US).contains("multipart/related"))
            {
                ct = ct + "; charset=" + enc;
            }
        }
    ...
    {
        boolean addHeaders = MessageUtils.isTrue(message.getContextualProperty(ADD_HEADERS_PROPERTY));
        for (String header : headers.keySet())
        {
            List<String> headerList = headers.get(header);
    ...
    private String getContentTypeFromMessage()
    {
        ...
        if (null != ct && null != enc && ct.indexOf("charset=") == -1
    ...
    protected void copyToResponse(HttpServletResponse response)
    {
        ...
        if (!headers.containsKey(Message.CONTENT_TYPE) && contentType != null && isResponseBodyAvailable())
        {
