...
public abstract class AbstractOutDatabindingInterceptor extends AbstractPhaseInterceptor<Message>
{
    ...
    protected void writeParts(Message message, Exchange exchange, BindingOperationInfo operation,
        MessageContentsList objs, List<MessagePartInfo> parts)
    {
        ...
        // need to cache the events in case validation fails or buffering is
        // enabled
        ...
        if (out != null && writeToOutputStream(message, operation.getBinding(), service)
    ...
    {
        /**
         * Yes, all this code is EXTREMELY ugly. But it gives about a 60-70%
         * performance boost with the JAXB RI, so its worth it.
         */
        ...
        return info.getClass().getName().equals("org.apache.cxf.binding.soap.model.SoapBindingInfo")
            ...
            && !MessageUtils.isDOMPresent(m) && (enc == null || "UTF-8".equals(enc));
    }
    ...
    protected <T> DataWriter<T> getDataWriter(Message message, Service service, Class<T> output)
    {
        ...
        if (MessageUtils.isTrue(message.getContextualProperty(org.apache.cxf.message.Message.MTOM_ENABLED))
            && atts == null)
        {
    ...
    private void setSchemaOutMessage(Service service, Message message, DataWriter<?> writer)
    {
        ...
        {
            Schema schema =
                EndpointReferenceUtils.getSchema(service.getServiceInfos().get(0), message.getExchange().getBus());
